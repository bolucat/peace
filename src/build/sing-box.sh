...
for ARCH in ${ARCHS[@]}; do
	if [ "${ARCH}" == "arm" ]; then
		...
		env CGO_ENABLED=0 GOOS=linux GOARCH=${ARCH} go build -o ${CUR}/release/sing-box-linux-${ARCH} -trimpath -tags 'with_quic,with_grpc,with_wireguard,with_shadowsocksr,with_ech,with_utls,with_acme,with_clash_api,with_gvisor' -ldflags "-X github.com/sagernet/sing-box/constant.Commit=${COMMIT} -w -s -buildid=" ./cmd/sing-box
...
env CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -o ${CUR}/release/sing-box.exe -trimpath -tags 'with_quic,with_grpc,with_wireguard,with_shadowsocksr,with_ech,with_utls,with_acme,with_clash_api,with_gvisor' -ldflags "-X github.com/sagernet/sing-box/constant.Commit=${COMMIT} -w -s -buildid=" ./cmd/sing-box
