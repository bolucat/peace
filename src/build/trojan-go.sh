...
git clone -b ${VERSION} https://github.com/p4gefau1t/trojan-go.git && cd trojan-go
...
for ARCH in ${ARCHS[@]}; do
    if [ "${ARCH}" == "arm" ]; then
        for ARM in ${ARMS[@]}; do
            ...
            env CGO_ENABLED=0 GOOS=linux GOARCH=${ARCH} GOARM=${ARM} go build -o ${CUR}/release/trojan-go-linux-${ARCH}32-v${ARM} -trimpath -tags "full" -ldflags "-s -w"
        ...
        env CGO_ENABLED=0 GOOS=linux GOARCH=${ARCH} go build -o ${CUR}/release/trojan-go-linux-${ARCH} -trimpath -tags "full" -ldflags "-s -w"
